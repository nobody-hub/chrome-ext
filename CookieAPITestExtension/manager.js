...
class CookieCache {
    ...
    remove(cookie) {
        ...
        if(this.cookies[domain]) {
            var i = 0;
            while (i < this.cookies[domain].length) {
                if (matchCookie(this.cookies_[domain][i], cookie)) {
    ...
    getDomains(filter) {
        var result = [];
        sortedKeys(this.cookies).forEach(function(domain) {
...
function removeAllForFilter() {
    ...
    var timer = new Timer();
    cache.getDomain(filter).forEach(function(domain) {
